...
Sparsity controls the rarity of the word frequency.  If we run `removeSparseTerms(tdm, sparse = 0.99)`, it will remove only the rarest words, that is the words that appear in only 1% of the corpus.  On the other hand, `removeSparseTerms(tdm, sparse = 0.01)` then only words that appear in nearly every document of the corpus will be kept.

Source is here: https://stats.stackexchange.com/questions/160539/is-this-interpretation-of-sparsity-accurate

To graph it, I summed up the rows to eliminate all the duplicate words, essentially removing the document identifier.
...
Attempt 2 collects each job post by city, then makes a corpus of each city's job postings.
I can't seem to manipulate the sparsity to showcase any relevant skills.  I suppose the more interesting thing is that, when grouped by city, the companies become the most important term.
...
Attempt-3 breaks down the documents by job titles.